...
resource "aws_api_gateway_rest_api" "main" {
  name        = "demo-quantum-api-${random_string.suffix.result}"
  description = "Quantum-Enhanced Serverless Optimization API"
...
resource "aws_api_gateway_integration" "lambda_integration" {
  ...
  type                   = "AWS_PROXY"
  uri                    = aws_lambda_function.quantum_optimizer.invoke_arn
}
...
resource "aws_api_gateway_stage" "main" {
  ...
  tags = {
    Name = "demo-quantum-stage-${random_string.suffix.result}"
  }
}

# API Gateway Usage Plan (more restrictive for quantum)
resource "aws_api_gateway_usage_plan" "main" {
  name         = "demo-quantum-usage-plan-${random_string.suffix.result}"
  description  = "Usage plan for quantum serverless API (limited due to higher costs)"
...
resource "aws_api_gateway_api_key" "main" {
  name        = "demo-quantum-api-key-${random_string.suffix.result}"
  description = "API key for quantum serverless optimization"
